...
# Subscript ₁ = one octave below home
# Superscript ' = one octave above home
# ═══════════════════════════════════════════════════════
...
VOICE_OCTAVE_RANGES = {
    1: {  # Soprano — high
        'typical': (4, 5),      # C4–C5 (unmarked and superscript)
        'extended_low': 3,       # Can reach C3 (subscript ₁)
        'extended_high': 6,      # Can reach C6 (superscript '')
    },
    2: {  # Alto — medium–high
        'typical': (4,),         # C4–B4 (unmarked)
        'extended_low': 3,       # Often dips to C3 (subscript ₁)
        'extended_high': 5,      # Can reach C5 (superscript ')
    },
    3: {  # Tenor — medium–low
        'typical': (3, 4),       # C3–B4 (mostly subscript ₁, some unmarked)
        'extended_high': 5,      # Rare high notes in superscript '
    },
    4: {  # Bass — low
        'typical': (2, 3),       # C2–C3 (subscript ₂ and ₁)
...
# Output style for low octaves uses compact Unicode subscript digits.
SUBSCRIPT_DIGITS = {
    '0': '₀', '1': '₁', '2': '₂', '3': '₃', '4': '₄',
    '5': '₅', '6': '₆', '7': '₇', '8': '₈', '9': '₉'
}
...
def octave_mark(n:int)->str:
    ...
    if n > 0:
        return "'" * n
    # negative => Unicode subscript digits
    return ''.join(SUBSCRIPT_DIGITS.get(c,c) for c in str(abs(n)))
...
def _solfa_octave_suffix(octave:int, middle:int=4)->str:
    ...
    if diff>0:
        return "'"*diff
    return ''.join(SUBSCRIPT_DIGITS.get(c,c) for c in str(abs(diff)))


def _note_display_symbol(note, key:str='C')->str:
    if note.rest:
        return ''  # Empty space for rests
    solfa_sym = note.solfa(key)
    # Add duration marker
    dur_marker = note.duration_underscores()
    return solfa_sym + dur_marker
...
class MusNote:
    ...
    def solfa(self,key:str='C')->str:
        ...
        Uses home note system per key and voice.
        """
        if self.rest:
            return ''  # Empty space for rests
        ...
        home_octave = get_home_octave(key, self.voice)
        octave_offset = self.octave - home_octave

        if octave_offset < 0:
            syl += ''.join(SUBSCRIPT_DIGITS.get(ch, ch) for ch in str(abs(octave_offset)))
...
class TraditionalSolfaCanvas(tk.Canvas):
    ...
    def __init__(self,master,score:Score,**kwargs):
        ...
        self.measure_width=200  # adjustable measure width
...
class TonicSolfaStudio(tk.Tk):
    ...
    def _apply_measure_resize_from_panel(self):
        ...
        if not hasattr(self,'staff_canvas'): return
        ...
        self.staff_canvas.MEAS_W = self.measure_width
        if hasattr(self, 'solfa_canvas'):
            self.solfa_canvas.measure_width = self.measure_width
            self.solfa_canvas.beat_width = self.beat_width
        self.status_var.set("Measure resize applied.")
        self.staff_canvas.redraw()
        if hasattr(self, 'solfa_canvas'):
            self.solfa_canvas.redraw()
        self._save_settings()
